( function () {
	...
	// Update the accordion header badge when the answer-type radio changes
	document.addEventListener( 'change', function ( e ) {
		if ( ! e.target.classList.contains( 'fyt-type-radio' ) ) return;
		var item  = e.target.closest( '.fyt-item' );
		var badge = item && item.querySelector( '.fyt-type-badge' );
		if ( ! badge ) return;
		var isRadio = e.target.value === 'radio';
		badge.textContent = isRadio ? 'Single' : 'Multi';
		badge.className   = 'fyt-type-badge fyt-type-badge--' + e.target.value;
	} );
		...
		el.innerHTML =
			'<div class="fyt-item__header">' +
				'<button type="button" class="fyt-item__toggle" aria-expanded="false">' +
					...
					'<span class="fyt-type-badge fyt-type-badge--checkbox">Multi</span>' +
				'</button>' +
			...
			'<div class="fyt-item__body" hidden>' +
				...
				'<table class="form-table fyt-form-table">' +
					...
					'<tr>' +
						'<th>Answer Type</th>' +
						'<td>' +
							'<div class="fyt-type-toggle" role="group" aria-label="Answer type">' +
								'<input type="radio" name="' + esc( prefix ) + '[type]" value="radio" id="fyt-q-type-r-' + qIdx + '" class="fyt-type-radio">' +
								'<label for="fyt-q-type-r-' + qIdx + '" class="fyt-type-btn">' +
									'<span class="dashicons dashicons-marker" aria-hidden="true"></span>' +
									'Single answer (radio)' +
								'</label>' +
								'<input type="radio" name="' + esc( prefix ) + '[type]" value="checkbox" id="fyt-q-type-c-' + qIdx + '" class="fyt-type-radio" checked>' +
								'<label for="fyt-q-type-c-' + qIdx + '" class="fyt-type-btn">' +
									'<span class="dashicons dashicons-yes-alt" aria-hidden="true"></span>' +
									'Multiple answers (checkbox, up to 3)' +
								'</label>' +
							'</div>' +
							'<p class="description">Single: contributors pick exactly one answer. Multiple: they can pick up to 3.</p>' +
						'</td>' +
					'</tr>' +
				'</table>' +
